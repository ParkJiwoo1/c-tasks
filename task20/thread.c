...
/*
   ...
   I/O ready (by setting PAL_FD_ISSET true), and must call
...
thread_master_create (void)
{
	...
	m = (struct thread_master *) calloc (MTYPE_THREAD_MASTER,
			sizeof (struct thread_master));

	if (m == NULL)
	{
		errno = EINVAL;
		perror ("thread_master_create");
		exit (1);
	}
 ...
 *
 * @param[in] list. thread list
 * @param[in] thread. thread will be trimed from list
 */
static struct thread *
thread_list_delete (struct thread_list *list, struct thread *thread)
{
	if (thread->next) /* thread is not tail */
		thread->next->prev = thread->prev;
	else
		list->tail = thread->prev;
	if (thread->prev) /* thread is not head */
		...
		int (*func) (struct thread *), void *arg)
{
	...
	if (m->unuse.head)
		thread = thread_trim_head (&m->unuse); /* get memory space(node) form unuse */
	else
	{
		thread = calloc (MTYPE_THREAD, sizeof (struct thread));
		if (thread == NULL)
			return NULL;
...
#ifdef HAVE_HA
	thread->period.tv_sec = timer;
	thread->period.tv_usec= 0;
	timeofday(&thread->stime, NULL);
#endif /* HAVE_HA */
		...
		struct timeval timer)
{
	...
	if (timer_now.tv_sec < 0)
		timer_now.tv_sec = TIME_MAX_TV_SEC;
	if (timer_now.tv_usec < 0)
		timer_now.tv_usec = TIME_MAX_TV_USEC;
...
thread_cancel (struct thread *thread)
{
	...
	thread->type = THREAD_UNUSED;
	thread_add_unuse (thread->master, thread); /* add to unuse queue */
}
...
thread_timer_wait (struct thread_master *m, struct timeval *timer_val)
{
	...
	/* selecet smallest timer in time slot */
	for (i = 0; i < THREAD_TIMER_SLOT; i++)
...
/**
 * set thread and add thread to unuse list
 *
 * @param[in] m. master thread
 * @param[in] thread. thread
 * @param[in] fetch. therad will be copy to fetch
 */
struct thread *
...
thread_process_fd (struct thread_master *m, struct thread_list *list,
		sock_set_t *fdset, sock_set_t *mfdset)
{
	struct thread *thread;
	struct thread *next;
	int ready = 0;

	for (thread = list->head; thread; thread = next) /* all thread in list */
	{
		next = thread->next;

		if (FD_ISSET (THREAD_FD (thread), fdset)) /* fd of thread match */
		{
			FD_CLR(THREAD_FD (thread), mfdset); /* processed fd clear */
			thread_list_delete (list, thread);
			thread_enqueue_middle (m, thread); /* add to middle queue */
			ready++;
		}
	}
	return ready; /* number of matched fd */
}
